        ...
        <v-row>
          ...
          >
            <p class="empty-data-caption my-5">
              Ainda não há Produto cadastrados. Adicione uma Produto.
            </p>
          </v-col>
        </v-row>
        ...
        >
          ...
          >
            <Button
              ...
              @click="() => $router.push('/produtos/cadastrar')"
            >
              <span
                class="text-wrap"
                onclick=""
              >
                clique aqui para cadastrar seu primeiro Produto
              </span>
            </Button>
          </v-col>
        </v-row>
...
<v-col cols="4">
                <label for="categoria_id" class="small-label">Produto Categoria</label>
                <v-select id="categoria_id" v-model="filtro.categoria_id" bg-color="white" rounded variant="outlined" placeholder="Filtrar por Produto Categoria" required class="w-100 mt-5 pr-10" color="primary" item-value="id" item-title="nome" :items="produtoCategorias" />
              </v-col>
              </v-row>
              <v-row justify="end">
                <v-col
          ...
          <v-table class="bg-transparent w-100 mt-16">
            <thead>
              <tr>
                ...
                <th class="bolder-primary-color font-weight-bold table-head-border">Produto Categoria</th>
                <th class="bolder-primary-color font-weight-bold table-head-border">Data de Criação</th>
                <th class="bolder-primary-color font-weight-bold table-head-border">Última atualização</th>
              </tr>
            </thead>
            <tbody>
              <tr
                v-for="(produto, index) in response?.data"
                :key="index"
              >
                <td class="table-foot-border table-content-text" :title="produto?.id">
                           {{ produto?.id }}
                         </td>
                <td class="table-foot-border table-content-text" :title="produto?.nome">
                           {{ produto?.nome }}
                         </td>
                <td class="table-foot-border table-content-text" :title="produto?.categoria_id">
                           {{ produto?.categoria?.nome }}
                         </td>
                <td class="table-foot-border table-content-text" :title="produto?.created_at">
                           {{ formataDateTimeParaDataBrasileira(produto?.created_at) }}
                         </td>
                <td class="table-foot-border table-content-text" :title="produto?.updated_at">
                           {{ formataDateTimeParaDataBrasileira(produto?.updated_at) }}
                         </td>
              </tr>
            </tbody>
      ...
      >
        <v-alert type="info">
          Nenhum Produto encontrada com a sua busca. Ajuste os filtros e tente
          novamente
        </v-alert>
      </div>
    </v-container>
...
export default {
  ...
  data(): {
    filtro: IFiltrarProduto;
    initialAccess: boolean;
    produtoCategorias: IProdutoCategoria[];
    response: PagedResponse<IProduto>;
    openedPanel: number;
  ...
  computed: {
    filtroVazio() {
      return !this.filtro.nome && !this.filtro.categoria_id;
    },
  },
  async mounted() {
    try {
      ...
      this.errorMessage = "Houve um erro ao buscar dados de Produto. Tente novamente a operação e em caso de persistência do erro entre em contato com o suporte";
  ...
  methods: {
    ...
    async filtrar() {
      try {
        ...
        this.isLoading = true;
        this.response = await ProdutoService.filter(this.filtro);
      } catch (error) {
        this.errorMessage = "Houve um erro ao buscar dados de Produto. Tente novamente a operação e em caso de persistência do erro entre em contato com o suporte";
        this.errorSnackbar = true;
      } finally {
