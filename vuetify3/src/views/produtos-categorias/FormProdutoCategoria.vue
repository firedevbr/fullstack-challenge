...
export default {
  ...
  data(): {
    ...
    formData: IFormProdutoCategoria
  } {
    return {
      isLoading: false,
      snackbar: false,
  ...
  async mounted() {
    try {
      ...
      this.errorMessage =
        'Erro ao buscar dados de Produto Categoria. Tente novamente e em caso de persistência de erros entre em contato com o suporte.';
    } finally {
  ...
  methods: {
    goBack() {
      this.$router.push('/produtos-categorias');
    },
    async saveProdutoCategoria() {
      this.isLoading = true;
      this.formErrors = {};
      ...
      try {
        ...
        let response;
        if (this.id) {
          response = await ProdutoCategoriaService.update({
            nome: this.formData.nome
          }, this.id);
        } else {
          response = await ProdutoCategoriaService.save(this.formData);
          this.id = response.id;
        }

        this.successMessage = "Produto Categoria salva com sucesso!";
        this.snackbar = true;
        ...
        if (error.response && error.response.status === 422) {
          ...
          this.errorSnackbar = true;
          this.errorMessage = "Houve um erro ao cadastrar Produto Categoria, valide os dados e tente novamente";
    ...
    async checkIfEditing() {
      try {
        ...
        if (this.$route?.params?.id) {
          ...
          const produtoCategoria = await ProdutoCategoriaService.getById(this.id);
          Object.assign(this.formData, produtoCategoria);
        }
