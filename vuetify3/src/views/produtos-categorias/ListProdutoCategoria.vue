<template>
  <v-container>
        ...
        <v-row>
          ...
          >
            <p class="empty-data-caption my-5">
              Ainda não há Produto Categoria cadastrados. Adicione uma Produto Categoria.
            </p>
          </v-col>
        </v-row>
        ...
        >
          ...
          >
            <Button
              ...
              @click="() => $router.push('/produtos-categorias/cadastrar')"
            >
              <span
                class="text-wrap"
                onclick=""
              >
                clique aqui para cadastrar seu primeiro Produto Categoria
              </span>
            </Button>
          </v-col>
        </v-row>
    ...
    >
      ...
      >
        ...
        <div class="scroll-table-container">
          <v-table class="bg-transparent w-100 mt-16">
            ...
            <tbody>
              <tr
                v-for="(produtoCategoria, index) in response?.data"
                :key="index"
              >
                <td class="table-foot-border table-content-text" :title="produtoCategoria?.id">
                           {{ produtoCategoria?.id }}
                         </td>
                <td class="table-foot-border table-content-text" :title="produtoCategoria?.nome">
                           {{ produtoCategoria?.nome }}
                         </td>
                <td class="table-foot-border table-content-text" :title="produtoCategoria?.created_at">
                           {{ formataDateTimeParaDataBrasileira(produtoCategoria?.created_at) }}
                         </td>
                <td class="table-foot-border table-content-text" :title="produtoCategoria?.updated_at">
                           {{ formataDateTimeParaDataBrasileira(produtoCategoria?.updated_at) }}
                         </td>
              </tr>
            </tbody>
              ...
              <td class="table-foot-border text-center table-content-text">
                <i
                  ...
                  @click="
                    () =>
                      $router.push(
                        `/produtos-categorias/detalhes/${produtoCategoria.id}`
                      )
                  "
                />
                <i
                  ...
                  @click="
                    () =>
                      $router.push(
                        `/produtos-categorias/editar/${produtoCategoria.id}`
                      )
                  "
                />
              </td>
      ...
      >
        <v-alert type="info">
          Nenhum Produto Categoria encontrada com a sua busca. Ajuste os filtros e tente
          novamente
        </v-alert>
      </div>
    </v-container>
...
export default {
  ...
  data(): {
    filtro: IFiltrarProdutoCategoria;
    initialAccess: boolean;
    
    response: PagedResponse<IProdutoCategoria>;
    openedPanel: number;
  ...
  async mounted() {
    try {
      ...
      this.errorMessage = "Houve um erro ao buscar dados de Produto Categoria. Tente novamente a operação e em caso de persistência do erro entre em contato com o suporte";
  ...
  methods: {
    ...
    async filtrar() {
      try {
        ...
        this.isLoading = true;
        this.response = await ProdutoCategoriaService.filter(this.filtro);
      } catch (error) {
        this.errorMessage = "Houve um erro ao buscar dados de Produto Categoria. Tente novamente a operação e em caso de persistência do erro entre em contato com o suporte";
        this.errorSnackbar = true;
      } finally {
